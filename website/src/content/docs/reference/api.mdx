...
| [panelClass]                | `string` \| `string[]` \| `Set<string>` \| `Record<string, boolean>` | `-`                 | Additional CSS classes applied to the dropdown panel. Merged with mirrored host `class`, `[class]`, and `[ngClass]` values. Preferred for panel-only styling.                                  |
