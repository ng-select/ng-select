        ...
        return 'styles: ['
            + urls.map(styleUrl => {
                ...
                if (!styleUrl.endsWith('.scss')) {
                    throw new Error('only scss styles are supported');
                }
                const styleFile = urlResolver(styleUrl);
                const styleContentSass = fs.readFileSync(styleFile, 'utf-8');
                const styleCompiled = sass.renderSync({data: styleContentSass});
                const styleContent = styleCompiled.css.toString();
